-- ============================================================================
-- Workspace Schema - Permissions Baseline
-- Date: 2024-12-14
-- Purpose: Grant all necessary permissions to handled_user for workspace schema
-- ============================================================================
-- Ensures handled_user has access to all objects in workspace schema,
...
-- =============================================================================
-- WORKSPACE SCHEMA PERMISSION GRANTS
-- =============================================================================
...
BEGIN
  ...
  LOOP
    -- Grant on tables
    EXECUTE format('ALTER DEFAULT PRIVILEGES FOR ROLE %I IN SCHEMA workspace GRANT ALL ON TABLES TO handled_user', 
                   admin_role);
    
    -- Grant on sequences
    EXECUTE format('ALTER DEFAULT PRIVILEGES FOR ROLE %I IN SCHEMA workspace GRANT ALL ON SEQUENCES TO handled_user', 
                   admin_role);
    
    -- Grant on functions
    EXECUTE format('ALTER DEFAULT PRIVILEGES FOR ROLE %I IN SCHEMA workspace GRANT ALL ON FUNCTIONS TO handled_user', 
                   admin_role);
    
    RAISE NOTICE 'Set default privileges for role % in schema workspace', admin_role;
  END LOOP;
END $$;
...
BEGIN
  ...
  IF NOT missing_perms THEN
    RAISE NOTICE '✓ Workspace schema permission grants verified successfully';
  END IF;
END $$;
