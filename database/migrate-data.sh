#!/bin/bash
#
# DATA Database Migration Runner
# 
# IMPORTANT: Migrations run as handled_user (from DATA_DATABASE_URL)
# This ensures consistent object ownership and permission grants
#
# Usage:
#   ./migrate-data.sh          # Run pending migrations
#   ./migrate-data.sh --force  # Force mode (skip some safety checks)
#   ./migrate-data.sh --repair # Repair mode (fix schema mismatches)
#   ./migrate-data.sh --verify # Verify permissions after migration
#
# Database: DATA DB (VPS Local)
# Contains: workspace schema + reference schema
...
# Configuration
MIGRATIONS_DIR="$(dirname "$0")/migrations-data"
FORCE_MODE=false
REPAIR_MODE=false
VERIFY_MODE=false
...
# Use DATA_DATABASE_URL or fall back to PRIMARY_DATABASE_URL for single-DB mode
if [ -z "$DATA_DATABASE_URL" ] && [ -z "$PRIMARY_DATABASE_URL" ]; then
    echo "Error: Neither DATA_DATABASE_URL nor PRIMARY_DATABASE_URL is set"
    exit 1
fi

DB_URL="${DATA_DATABASE_URL:-$PRIMARY_DATABASE_URL}"
...
print_header "DATA DB Migration Runner (Workspace + Reference)"

print_info "Database: DATA (VPS Local)"
...
# Note: tracking lives in config schema even for DATA DB
...
EXPECTED_SCHEMAS=("workspace" "reference")
MISSING_SCHEMAS=()
...
if [ "$VALIDATION_FAILED" = true ]; then
    ...
    if [ "$APPLIED_COUNT" -gt 0 ]; then
        ...
        print_fix "bash database/migrate-data.sh --repair"
        echo ""
        
        if [ "$REPAIR_MODE" = true ]; then
            print_info "REPAIR MODE: Re-running schema creation migrations..."
            echo ""
            # Clear tracking for schema creation migrations so they'll re-run
            psql "$DB_URL" -tAc \
                "DELETE FROM config.schema_migrations WHERE version IN ('001', '002', '003');" > /dev/null
            print_success "Cleared tracking for DATA migrations - will re-run"
            echo ""
        else
...
# Print summary
print_header "Migration Summary (DATA DB)"
...
if [ "$PENDING_COUNT" -eq 0 ]; then
    print_success "DATA database is up to date! No migrations needed."
else
...
if [ "$ALL_SCHEMAS_OK" = true ]; then
    print_header "✨ DATA DB Migrations Complete!"
else
    echo ""
...
if [ "$VERIFY_MODE" = true ]; then
    ...
    print_info "Checking table permissions for handled_user in workspace and reference schemas..."
    echo ""
...
FROM pg_tables 
WHERE schemaname IN ('workspace', 'reference')
...
" | tr -d ' ')
    ...
    if [ "$TOTAL_ISSUES" = "0" ]; then
        ...
        print_fix "  psql \"\$DB_URL\" -f database/migrations-data/005_ensure_db_permissions.sql"
        echo ""
        print_warning "Then verify again:"
        print_fix "  bash database/migrate-data.sh --verify"
    fi
fi
